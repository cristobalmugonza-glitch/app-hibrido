...
export type Tab = 'hoy' | 'registrar' | 'progreso' | 'nutricion' | 'mas';

const TABS: { id: Tab; nombre: string }[] = [
  { id: 'hoy', nombre: 'Hoy' },
  { id: 'registrar', nombre: 'Registrar' },
...
export default function App() {
  ...
  const abrirRegistro = () => setRegistroOculto(false);
  ...
  return (
    <div className="mx-auto min-h-[100dvh] max-w-md">
      <main className="px-5 pb-[calc(5rem+env(safe-area-inset-bottom))] pt-[calc(1.25rem+env(safe-area-inset-top))]">
        {tab === 'hoy' && <Hoy onIr={setTab} onAbrirRegistro={abrirRegistro} />}
        {tab === 'registrar' && <Registrar onAbrirRegistro={abrirRegistro} />}
